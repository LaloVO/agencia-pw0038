  ...
  return (
    <>
      ...
      <div className="bg-[#05121E] text-white min-h-screen relative font-sans selection:bg-[#C09A6F] selection:text-[#05121E]">
        ...
        <main className="pt-[72px] h-screen flex overflow-hidden relative z-10">
          {/* Map column */}
          <div className={cn(
            ...
            <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 max-w-[95vw]">
              ...
              />
              {(qParam || actionParam) && (
                <div className="flex flex-wrap gap-1.5 bg-[#07192A]/90 backdrop-blur-md border border-white/10 p-2 rounded-2xl shadow-elegant w-fit max-w-[90vw]">
                  {qParam && (
                    <span className="flex items-center gap-1.5 bg-[#C09A6F]/20 text-[#C09A6F] text-[10px] font-sans font-bold uppercase tracking-wider px-3 py-1 rounded-full border border-[#C09A6F]/30">
                      Búsqueda: {qParam}
                      <button
                        type="button"
                        onClick={() => {
                        ...
                        className="hover:text-white transition-colors ml-1 font-extrabold text-xs"
                      >
                        ×
                      </button>
                    </span>
                  ...
                  <button
                    ...
                    className="text-[9px] uppercase tracking-widest font-extrabold text-[#9BB0C1] hover:text-[#C09A6F] px-2 py-1 transition-colors"
                  >
                    Limpiar Búsqueda
                  </button>
                </div>
          ...
          <aside className={cn(
            "flex-col w-full lg:w-96 border-l border-white/5 bg-[#05121E] text-white shrink-0 h-full",
            viewMode === 'list' ? 'flex' : 'hidden lg:flex'
          )}>
            <div className="px-5 py-4 border-b border-white/5 bg-[#07192A]/40 backdrop-blur-md">
              <h1 className="font-sans font-bold text-lg text-white tracking-wide">Propiedades</h1>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-24 lg:pb-4">
              ...
              {!isLoading && filtered.length === 0 && (
                <div className="text-center py-16">
                  <p className="font-sans text-sm text-[#9BB0C1]">Sin resultados</p>
                  <p className="text-xs text-[#9BB0C1]/60 mt-1">Ajusta los filtros o limpia tu búsqueda</p>
                </div>
              )}
            </div>
          </aside>
          ...
          <button
            ...
            className="lg:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-40 flex items-center gap-2 px-5 py-3 rounded-full bg-[#07192A] backdrop-blur border border-white/10 text-white font-sans text-xs font-bold uppercase tracking-wider shadow-elegant hover:bg-[#07192A]/80 active:scale-95 transition-all cursor-pointer"
          >
            {viewMode === 'map' ? (
              <>
                <List className="w-4 h-4 text-[#C09A6F]" />
                <span>Ver Lista</span>
              </>
            ) : (
              <>
                <MapIcon className="w-4 h-4 text-[#C09A6F]" />
                <span>Ver Mapa</span>
              </>
            )}
          </button>
